...
function PostgresCouchDB(pgclient, options) {
        ...
        var previous_since = opts.couchdb.since;
        var alive = true;
        var status = '';
	...
	setTimeout(function() {
        ...
        function checkpoint_changes() {
	    var ckwait = 3 * 1000;
	    if(alive){
		if(previous_since < changecount){
	            pgclient.query("UPDATE since_checkpoints SET since=" + changecount + " WHERE pgtable='" + pgtable + "'", function(err, result) {
        	        if (err) {
                            //catch postgres disconnects and other errors?
                	    events.emit('checkpoint.error', pgtable + ": UNABLE TO SET SINCE CHECKPOINT to " + changecount, err);
	                } else {
	                    events.emit('checkpoint', pgtable + ': Checkpoint set to ' + changecount + ' next check in ' +  (ckwait / 1000) + " seconds");
	                }
	            });
		}else{
	      //       ckwait = Math.floor(Math.random() * ((60000*2) - 30000) + 30000)
		     ckwait = 10 * 1000; //increase wait as may be idle for a while - adjust to suite needs
	             events.emit('checkpoint', pgtable + ": Checkpoint " + changecount + ' is current next check in: ' +  Math.floor(ckwait / 1000) + ' seconds');
		}
		previous_since = changecount;
		setTimeout(function() {
			checkpoint_changes()
		     }, ckwait);
	    }
                        ...
                        if (doc._rev != result.rows[0].rev) {		     
	                        ...
	                        pgclient.query(sql, function(err, result) {
                        	    if (err) {
	                                ...
	                                deferred.resolve(doc.rev);
        	                    }
	                        });

                        }
                        /* else {
                            if (doc_rev_num == pg_rev_num) {
                                //no updates to docs in pg so all of _rev should be the same
                                //but lets check to make sure
                                if (doc._rev != result.rows[0].rev) {
                                    //we have a problem 
                                    console.log(pgtable + ": rev mismatch for doc: " + key + " " + doc.type);
                                    //deferred.reject(err); //maybe raise error
                                }
                                //console.log(pgtable + ": NOOP " + doc._id + " pg_rev: " + pg_rev_num + " = doc_rev: " + doc_rev_num);
                                deferred.resolve(result.rows[0].rev);

                            } else {
                                //shouldnt get here as any inserted record should be current one from couch
                                console.log(pgtable + ": NOOP " + doc._id + " pg_rev: " + pg_rev_num + " > doc_rev: " + doc_rev_num);
                                deferred.resolve(result.rows[0].rev);
                            }
                        } */
...
//                     stopFollowing();
        ...
        function startFollowing() {
            ...
            stream.since = opts.couchdb.since;
